...
| Audit events, outbox, webhook delivery | Partial, not-production-stable | Service README, webhook receiver reference, outbox tests, audit-repair/dead-letter replay evidence, multi-node active-lease evidence, `sha256-v1` audit hash chain, retention checkpoints, integrity endpoint, and SIEM detection examples. | Real multi-node failover, external anchor export and SIEM exporter integration. |
