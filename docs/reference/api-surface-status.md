...
| --- | --- | --- | --- |
| Identity, issuer, profile, enrollment, approval, issuance | Implemented, not-production-stable | Service README, OpenAPI, state transitions, Go/C++ boundary tests, and multi-node issuance-claim evidence. | Public API compatibility freeze and infrastructure-level primary-failover evidence. |
...
| CRL publication and OCSP response | Implemented, not-production-stable | Service README, core CLI contract, CRL/OCSP tests, signer outage/recovery evidence, and multi-node CRL lease evidence. | Infrastructure-level HA deployment, managed-primary failover and load-balancer traffic-shift evidence. |
...
| Audit events, outbox, webhook delivery | Partial, not-production-stable | Service README, webhook receiver reference, outbox tests, audit-repair/dead-letter replay evidence, multi-node active-lease evidence, `sha256-v1` Audit chain, integrity API, migration/backfill and recovery evidence, SIEM detection examples. | Infrastructure-level primary failover, independently anchored chain export and SIEM exporter integration. |
...
| HSM/KMS/PKCS#11 signing boundary | Partial | ADR 0007, key-provider semantics, certificate/CRL/OCSP FileKeyProvider tests, ECDSA and encrypted-PEM regression coverage, a single-provider resolver, test-only software-token contract coverage, signing-result audit correlation, production rejection, and source-boundary validation. | Real non-exportable provider support remains outside the current Community runtime scope. |
