...
Audit events include structured `metadata_json` for lifecycle resource IDs and successful result codes. Enrollment and issuance policy events record an allow/reject/unknown decision, a stable reason, and redacted validation-evidence references bound to identity/issuer/profile IDs plus SHA-256 CSR and policy-input references; raw CSR, SAN, key, and credential values are not copied into the references. HTTP requests can attach `X-Request-ID`; the service records it with the client IP for mutating operations. Every event is assigned an immutable `chain_index` and `sha256-v1` hash linked to the previous event. `GET /audit-events/integrity` verifies retained rows, the latest retention checkpoint, and the persisted tail state without rewriting broken hashes. `GET /audit-events` supports exact filters for actor, action, resource type, resource ID, RFC3339 created time range, ascending/descending sort, and limit/offset pagination. Retention pruning removes only a contiguous expired prefix, writes an anchor checkpoint, and then records an `audit.retention_pruned` event.
