...
"""Self-checks for the intermediate multi-node reliability drill."""
...
RUNNER = ROOT / "scripts" / "verify-multi-node-reliability.py"
SPEC = importlib.util.spec_from_file_location("multi_node_reliability_drill", RUNNER)
if SPEC is None or SPEC.loader is None:
    raise SystemExit("unable to load multi-node reliability drill")
...
def main() -> None:
    with tempfile.TemporaryDirectory() as dirname:
        ...
        written = json.loads((root / "out" / "multi-node-verification.json").read_text(encoding="utf-8"))
    ...
    print("multi-node reliability drill tests passed: 5")
