...
"""Run the Community multi-node reliability verification drill."""
...
DEFAULT_OUTPUT = ROOT / ".tmp" / "multi-node-evidence"
...
def write_evidence(output_dir: Path, evidence: dict[str, Any]) -> None:
    ...
    for forbidden in FORBIDDEN_EVIDENCE_TEXT:
        if forbidden in lowered:
            raise DrillFailure("forbidden sensitive content in multi-node reliability evidence")
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "multi-node-verification.json").write_text(serialized, encoding="utf-8")
    lines = [
        "# AnoPKI Community Multi-Node Reliability Evidence",
        "",
...
def run_drill(root: Path, output_dir: Path, go_executable: str, commit: str) -> dict[str, Any]:
    ...
    environment["GOCACHE"] = str(root / ".tmp" / "go-cache" / "multi-node-build")
    environment["GOMODCACHE"] = str(root / ".tmp" / "go-cache" / "multi-node-mod")
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "multi-node-test.log").write_text(redact(test_result.stdout, output_dir), encoding="utf-8")
    ...
    evidence["blocker"] = "" if passed else "one or more required multi-node reliability tests failed or were missing"
    write_evidence(output_dir, evidence)
    return evidence
...
def main() -> int:
    ...
    if evidence["result"] == "passed":
        print(f"multi-node reliability drill passed: {len(EXPECTED_CHECKS)} checks")
        return 0
