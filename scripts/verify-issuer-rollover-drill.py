...
"""Run the Community intermediate issuer rollover and rollback verification drill."""
...
DEFAULT_OUTPUT = ROOT / ".tmp" / "issuer-rollover-evidence"
...
def write_evidence(output_dir: Path, evidence: dict[str, Any]) -> None:
    ...
    for forbidden in FORBIDDEN_EVIDENCE_TEXT:
        if forbidden in lowered:
            raise DrillFailure("forbidden sensitive content in issuer rollover evidence")
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "issuer-rollover-verification.json").write_text(serialized, encoding="utf-8")
    lines = [
        "# AnoPKI Community Intermediate Issuer Rollover Evidence",
        "",
    ...
    (output_dir / "issuer-rollover-verification.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def run_drill(root: Path, output_dir: Path, go_executable: str | list[str], commit: str) -> dict[str, Any]:
    ...
    environment["GOCACHE"] = str(root / ".tmp" / "go-cache" / "issuer-rollover-build")
    environment["GOMODCACHE"] = str(root / ".tmp" / "go-cache" / "issuer-rollover-mod")
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "issuer-rollover-test.log").write_text(redact(test_result.stdout, output_dir), encoding="utf-8")
    ...
    evidence["blocker"] = "" if passed else "one or more required issuer rollover tests failed or were missing"
    write_evidence(output_dir, evidence)
    return evidence
...
def main() -> int:
    ...
    if evidence["result"] == "passed":
        print(f"issuer rollover drill passed: {len(EXPECTED_CHECKS)} checks")
        return 0
