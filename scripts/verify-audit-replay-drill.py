...
"""Run the Community audit-repair and dead-letter replay verification drill."""
...
DEFAULT_OUTPUT = ROOT / ".tmp" / "audit-replay-evidence"
...
EXPECTED_TESTS = (
    ...
    (
        ...
        "TestRepairMissingIssuanceAuditEvents",
    ),
    (
        "github.com/anothel/anopki/service/internal/httpapi",
        "TestReplayDeadLetterOutboxMessagesRecoversAfterOperatorReplay",
    ),
)
...
def write_evidence(output_dir: Path, evidence: dict[str, Any]) -> None:
    ...
    for forbidden in FORBIDDEN_EVIDENCE_TEXT:
        if forbidden in lowered:
            raise DrillFailure("forbidden sensitive content in audit/replay evidence")

    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "audit-replay-verification.json").write_text(
        serialized, encoding="utf-8"
    )

    lines = [
        "# AnoPKI Community Audit Repair And Dead-Letter Replay Evidence",
        "",
    ...
    (output_dir / "audit-replay-verification.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
...
def run_drill(
    ...
    environment["GOCACHE"] = str(root / ".tmp" / "go-cache" / "audit-replay-build")
    environment["GOMODCACHE"] = str(root / ".tmp" / "go-cache" / "audit-replay-mod")
    ...
    evidence["go_version"] = (
        redact(version_result.stdout.strip(), output_dir) or "unavailable"
    )
    ...
    if version < MINIMUM_GO_VERSION:
        evidence["blocker"] = (
            f"unsupported Go version {version[0]}.{version[1]}.{version[2]}; "
            "minimum is 1.25.11"
        )
        write_evidence(output_dir, evidence)
        return evidence
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    (output_dir / "audit-replay-test.log").write_text(
        redact(test_result.stdout, output_dir), encoding="utf-8"
    )
    ...
    evidence["tests"] = [
        {
            "package": package,
            "name": name,
            "status": observed.get((package, name), "missing"),
        }
        for package, name in EXPECTED_TESTS
    ]
    passed = test_result.returncode == 0 and all(
    ...
    evidence["blocker"] = (
        ""
        if passed
        else "one or more required audit-repair/dead-letter replay tests failed or were missing"
    )
    write_evidence(output_dir, evidence)
    return evidence
...
def main() -> int:
    ...
    if evidence["result"] == "passed":
        print(f"audit/replay drill passed: {len(EXPECTED_CHECKS)} checks")
        return 0
