...
def require_release_workflow() -> None:
    ...
    required = [
        ...
        "anopki-status-outage-verification.tar.gz", "anopki-audit-replay-verification.tar.gz", "anopki-issuer-rollover-verification.tar.gz", "anopki-postgres-recovery-verification.tar.gz", "anopki-multi-node-verification.tar.gz", "anopki-audit-chain-verification.tar.gz",
        "python scripts/verify-recovery-drill.py", "python scripts/verify-status-outage-drill.py", "python scripts/verify-audit-replay-drill.py", "python scripts/verify-issuer-rollover-drill.py", "python scripts/verify-postgres-recovery-drill.py", "python scripts/verify-multi-node-reliability.py", "python scripts/verify-audit-hash-chain.py",
...
def test_missing_audit_hash_chain_runner_fails(tmp_path: Path) -> None:
    copy_release_evidence_inputs(tmp_path)
    mutate(tmp_path / "scripts/verify-audit-hash-chain.py", "audit-chain-tamper-detection")
    result = run_validator(tmp_path)
    assert result.returncode == 1
    assert "audit-chain-tamper-detection" in result.stderr


def test_missing_release_audit_hash_chain_evidence_fails(tmp_path: Path) -> None:
    copy_release_evidence_inputs(tmp_path)
    mutate(tmp_path / ".github/workflows/release.yml", "anopki-audit-chain-verification.tar.gz")
    result = run_validator(tmp_path)
    assert result.returncode == 1
    assert "anopki-audit-chain-verification.tar.gz" in result.stderr
...
def main() -> None:
    ...
    tests = [
        ...
        test_missing_audit_hash_chain_runner_fails,
        test_missing_release_audit_hash_chain_evidence_fails,
